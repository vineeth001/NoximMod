...
#define TRAFFIC_OneDest	       8		//new trafficOneDest	
...
#define DEFAULT_ROUTING_ALGORITHM           ROUTING_XY		//change1
...
class NoximCoord {
  public:
    ...
    inline bool operator ==(const NoximCoord & coord) const {  //to check if the coordinates are equal
...
struct NoximPacket {
    int src_id;
    int dst_id;
    int inter_dst_id;		// vinu: intermediate_destination_id : only for valiants
    double timestamp;		// SC timestamp at packet generation
    int size;			// in terms of flits
    int flit_left;		// Number of remaining flits inside the packet

    // Constructors
    NoximPacket() { }
    NoximPacket(const int s, const int d, const int inter_d, const double ts, const int sz) {		//vinu: added inter_d
	make(s, d, inter_d, ts, sz);
    }

    void make(const int s, const int d, const int inter_d, const double ts, const int sz) {
	src_id = s;
	dst_id = d;
	inter_dst_id = inter_d;		//vinu: added inter_dst_id
...
struct NoximNoP_data {
    ...
    //compare sender_id and channel_status_neighbors
...
struct NoximFlit {
    int src_id;
    int dst_id;
    int inter_dst_id;		// vinu: intermediate destination (i_d_i)
	...
	return (flit.src_id == src_id && flit.dst_id == dst_id && flit.inter_dst_id == inter_dst_id 		//vinu: i_d_i
...
// Output overloading to display flit info
...
inline ostream & operator <<(ostream & os, const NoximFlit & flit)
{
	...
	os << "Destination Tile[" << flit.dst_id << "]" << endl;
        os << "Intermediate Destination Tile[" << flit.inter_dst_id << "]" << endl;
...
//TODO : need to recalculate hopcount etc
...
// Output overloading to channel status info
...
// Output overloading to NoP data
