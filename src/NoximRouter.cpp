...
void NoximRouter::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 1; i++) {
		...
		if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
			...
			ofstream trace_file;
			trace_file.open(file_name.c_str(),ios::out|ios::app);
			trace_file<< sc_time_stamp().to_double() /
    ...
    int >NoximRouter::routingFunction(const NoximRouteData & route_data)
{
    ...
    case ROUTING_XY:
	return routingXY(position, dst_coord);  		//vinu: add your own routing function
...
int NoximRouter::route(const NoximRouteData & route_data)
{
    ...
    if (route_data.dst_id == local_id)    //met the destination
...
vector < int >NoximRouter::routingXY(const NoximCoord & current,			//vinu: change3; also declare in .h
				     const NoximCoord & destination)		//print in rxprocess and txprocess and print in 
{
